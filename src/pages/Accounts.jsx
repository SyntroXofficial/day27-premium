          ...
          <div className="absolute inset-0 h-full flex items-center">
            <div className="w-[500px] pl-4">
              <div className="flex gap-2 mb-3">
                ...
                {services.find(s => s.items.some(i => i.name === featuredAccounts[currentSlide]?.name))
                  ?.items.find(i => i.name === featuredAccounts[currentSlide]?.name)?.status === 'inStock' ? (
                  <div className="inline-block px-4 py-1.5 rounded-[4px] text-sm font-semibold uppercase tracking-wide bg-[#000000] text-green-400" style={{ boxShadow: '0 1px 2px rgba(0, 0, 0, 0.2)' }}>
                    In Stock
                  </div>
                ) : (
                  <div className="inline-block px-4 py-1.5 rounded-[4px] text-sm font-semibold uppercase tracking-wide bg-[#000000] text-red-400" style={{ boxShadow: '0 1px 2px rgba(0, 0, 0, 0.2)' }}>
                    Out of Stock
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                {featuredAccounts[currentSlide]?.detailedInfo?.features?.slice(0, 4).map((feature, index) => (
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredAccounts.map((account, index) => (
            <div
              key={index}
              className="relative bg-gray-900 rounded-lg overflow-hidden hover:ring-2 hover:ring-red-500 transition-all cursor-pointer group"
